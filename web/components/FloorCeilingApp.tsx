...
const navItems: Array<{ id: TabId; label: string; icon: typeof LayoutDashboard }> = [
  { id: "video-submission", label: "Video Submission", icon: Video },
  { id: "overview", label: "Overview", icon: LayoutDashboard },
  { id: "methodology", label: "Methodology", icon: GitBranch },
  { id: "calibration", label: "Calibration", icon: Target },
  { id: "projection", label: "Forecast workflow", icon: Zap },
  { id: "overrides", label: "Manual overrides", icon: SlidersHorizontal },
];
...
function VideoSubmissionPage() {
  return (
    <div className="video-submission">
      <iframe
        className="video-submission-frame"
        src="https://www.youtube.com/embed/c65pWEHklk8"
...
function OverridesPage({ runLabel, rows, overrides, history, selectedRow, selectedPlayerId, onSelectRow, onSave, onResetPlayer, onResetAll, onCopy }: OverridesPageProps) {
  ...
  return (
    <>
      <SectionIntro eyebrow="Manual overrides" title="Model Overrides" status={<StatusPill label={`${Object.keys(overrides).length} saved`} tone={Object.keys(overrides).length ? "warn" : "neutral"} />} action={<div className="action-group"><Button variant="secondary" onClick={onCopy} icon={<CopyIcon />}>Copy prior overrides</Button><Button variant="quiet" onClick={onResetAll} icon={<RotateCcw size={15} />}>Reset all</Button></div>}>Model adjustments are not saved automatically. Must click &quot;Save overrides&quot; below.</SectionIntro>
