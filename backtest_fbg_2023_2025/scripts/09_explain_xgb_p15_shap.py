"""Explain the direct XGBoost PPR p15 models with Tree SHAP.

The experiment models raw PPR p15 points. This script explains those point
...
TARGET_QUANTILE = 0.15
QUANTILE_LABEL = "p15"


def load_experiment_module(root: Path) -> Any:
    """Load the P15 experiment helpers."""
    script_path = root / "backtest_fbg_2023_2025" / "scripts" / "08_xgb_p15_projection_experiment.py"
    spec = importlib.util.spec_from_file_location("xgb_p15_experiment", script_path)
...
def high_low_direction(values: pd.Series, shap_values: np.ndarray) -> dict[str, float | str]:
    ...
    if high_mean - low_mean > 0.10:
        direction = "higher values push p15 higher"
    elif low_mean - high_mean > 0.10:
        direction = "higher values push p15 lower"
    else:
        ...
        rows.append(
            {
                ...
                "target_quantile": TARGET_QUANTILE,
                "quantile_label": QUANTILE_LABEL,
                "baseline_p15": float(target.iloc[row_index]["p15"]),
                "model_p15": prediction,
...
def run(args: argparse.Namespace) -> None:
    """Run SHAP explanations for every selected P15 target model."""
    root = Path(__file__).resolve().parents[2]
    base_output = root / "backtest_fbg_2023_2025" / "outputs" / "xgb_p15_projection"
    ...
    if selected_models.empty:
        raise ValueError("No selected P15 models match the requested target seasons.")
    ...
    for model_row in selected_models.sort_values(["target_season", "position"]).itertuples(index=False):
        ...
        model_path = base_output / "models" / f"xgb_p15_{position.lower()}_through_{target_season - 1}.json"
        target = data[
        ...
        top_row_index = int(np.argmin(model_prediction))
        shap.plots.waterfall(explanation[top_row_index], max_display=args.top_features, show=False)
        plt.title(f"Lowest model p15, {label}")
        save_plot(plot_dir / f"waterfall_lowest_{position.lower()}_{target_season}.png")
        local_parts.append(
            local_rows(
                explanation,
                X,
        ...
        overprediction_index = int(np.argmax(model_prediction - target["actual_score"].to_numpy()))
        shap.plots.waterfall(explanation[overprediction_index], max_display=args.top_features, show=False)
        plt.title(f"Largest model minus actual p15, {label}")
        save_plot(plot_dir / f"waterfall_overprediction_{position.lower()}_{target_season}.png")
        local_parts.append(
            local_rows(
                explanation,
                X,
                ...
                position,
                target_season,
                "largest_model_minus_actual_p15",
                overprediction_index,
                args.top_features,
            )
        )
        model_checks.append(
            {
                ...
                "mean_model_p15": float(np.mean(model_prediction)),
    ...
    metadata = {
        ...
        "explainer": "shap.TreeExplainer",
        "feature_perturbation": "tree_path_dependent",
        "model_output": "raw PPR p15 points",
