# Direct p85 Model Experiments and SHAP

## Rule

Keep direct projection-based p85 experiments separate from the rank-conditioned
simulation baseline. Train and evaluate one quantile model per position with
season-level walk-forward splits, then compare each position against the
baseline with coverage and p85 pinball loss.
...
- Testing a direct p85 model with Footballguys or another projection source.
- Adding raw stat projections or derived projection features to a ceiling model.
...
- Use realized weekly `actual_score` as the target and a quantile objective
  with `quantile_alpha = 0.85`. Keep actual outcomes, player identifiers,
  names, teams, and target-season rows out of the feature set.
...
- Write separate model files, selected settings, predictions, metrics,
  ...
  output directory. Compare the candidate with the baseline for p85 coverage,
  p85 pinball loss, bias, rank correlation, and boom capture.
...
the candidate is stable across more seasons and improves the ceiling scorecard
without materially moving coverage away from 85 percent.
